...
public class Simplify {
    ...
    private static void simplify(Graph g) {
        ...
        for (var e : g.vertices.entrySet()) {
            var k = e.getKey();
            var v = e.getValue();
            if (v.neighbours.size() == 1) { // if edge only going to one vertix
                optimisable.add(k);
            }
            for (var i : v.neighbours.keySet()) { // map where a vertix appear
        ...
        for (var mid : optimisable) {
            ...
            int to = new ArrayList<>(g.vertices.get(mid).neighbours.keySet()).get(0);
            if (mid == to) {
                continue;
            }
    ...
    private static void biSimplify(Graph g) {
        ...
        for (var e : g.vertices.entrySet()) {
            var k = e.getKey();
            var v = e.getValue();
            if (v.neighbours.size() == 2) { // if edge only going to one vertix
                optimisable.add(k);
            }
            for (var i : v.neighbours.keySet()) { // map where a vertix appear
                var list = where.getOrDefault(i, new ArrayList<Integer>());
    ...
    private static boolean simplifyVertices(Graph g, int from, int mid, int to) {
        ...
        var cost1 = g.vertices.get(from).neighbours.get(mid);
        var cost2 = g.vertices.get(mid).neighbours.get(to);
        var cost = cost1.plus(cost2);
        removeEdge(g.vertices.get(from), mid);
